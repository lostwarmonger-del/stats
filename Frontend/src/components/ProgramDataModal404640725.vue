    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block w-full max-w-7xl p-6 my-8 overflow-hidden text-right align-middle transition-all transform bg-white shadow-xl rounded-2xl" @click.stop>
        ...
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-2xl font-bold text-gray-900">داده‌های ذخیره شده - برنامه 404640725</h3>
...
const deleteData = async (item) => {
  ...
  try {
    ...
    await axios.delete(`/programs/404640725/${item.id}`)
...
const exportData = async () => {
  try {
    ...
    const response = await axios.get('/programs/404640725/export', {
      responseType: 'blob'
    })
    ...
    link.href = url
    link.setAttribute('download', 'program_404640725_data.xlsx')
...
const fetchUsers = async () => {
  try {
    ...
    const response = await axios.get('/programs/404640725/all')
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await axios.get(`/programs/404640725/all?${params.toString()}`)
